# Bienvenido a mi Perfil de GitHub!

¡Hola! Soy Patricio Dunstan Saez, un Ingeniero en Maquinaria y Mecánica Automotriz y estudiante de Técnico Informático en IPP. Estoy en un proceso de reconversión, ya que al estudiar las bases de programación, me di cuenta de que es lo que realmente me gusta hacer. Desde ese día, no he detenido mi camino para crecer como desarrollador. Me encanta el mundo de la tecnología por su apertura a compartir información, la creación y la ayuda para solucionar problemáticas, y sobre todo, ayudar a las personas a resolver sus necesidades. Me gusta trabajar en ambientes colaborativos y en equipos, ya que creo que un buen equipo siempre marca y marcará la diferencia. Sobre todo, el apoyo entre compañeros y la capacidad de aceptar diferentes puntos de vista. Esto hace que al resolver un problema o necesidad, la solución sea la mejor posible.

## Mis Proyectos Destacados

1. https://github.com/patriciodunstan/portafolio : Pagina personal realizada con NextJs (SSR), TailwindCSS y Frame-Motion
2. https://github.com/patriciodunstan/node-CRUD-API - Proyecto de practica API CRUD utilizando NodeJs, Express, Docker, Docker Compose y Sequelize. 
3. https://github.com/patriciodunstan/c10-frontend - Desarrollador Front-End en proyecto de incubadora de desafio latam, cliente Devsafio, actualización y mejora de pagina de reclutamiento de jr para sector TI, utilizamos ReactJs-NextJs, TailwindCSS.

## Idiomas
-  ingles B2 Link: https://www.efset.org/cert/QocV6k

## Habilidades

- Lenguajes de Programación: JavaScript, Java
- Tecnologías: Angular , Typescript , Bootstrap, NodeJs, Express, AWS, Serverless

## Aprendizaje Continuo

Actualimente creando proyecto personal de plataforma que entrega indices financieros y webscraping para obtencion de data.

## Contacto

- LinkedIn: www.linkedin.com/in/patriciodunstan
- CV Drive: https://drive.google.com/file/d/1F6hzI3jlC7Qc6GhVZBn9eloPevhDu1CG/view?usp=drive_link

¡No dudes en explorar mis repositorios y proyectos! Si tienes alguna pregunta o sugerencia, ¡estaré encantado de escucharte!
